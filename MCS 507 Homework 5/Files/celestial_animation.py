# L-24 MCS 507 Mon 22 Oct 2012 : celestial_animation.py
...
# extending the script celestial.py with an animation.
...
def animate(z,nbframes):
   """
   Makes an animation of the trajectories
   computed by odeint in z, using nbframes.
   """
   x1 = z[:,0]; x2 = z[:,4]; x3 = z[:,8]
   y1 = z[:,2]; y2 = z[:,6]; y3 = z[:,10]
   n = len(x1); deltaframe = n/nbframes; frame = deltaframe
   plt.ion(); fig = plt.figure()
   ax = fig.add_subplot(111)
   ax.set_xlim(-1.5,1.5); ax.set_ylim(-0.5,0.5)
   for i in range(nbframes):
      s1x = x1[1:frame]; s1y = y1[1:frame]
      s2x = x2[1:frame]; s2y = y2[1:frame]
      s3x = x3[1:frame]; s3y = y3[1:frame]
      ax.plot(s1x,s1y,'r',s2x,s2y,'g',s3x,s3y,'b')
      fig.canvas.draw(); plt.pause(1)
      frame = frame + deltaframe
   ans = raw_input('hit enter to quit\n')
